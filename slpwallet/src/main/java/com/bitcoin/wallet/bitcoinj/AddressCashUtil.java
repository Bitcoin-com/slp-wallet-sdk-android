...
public class AddressCashUtil {
    ...
    public static byte[] decodeCashAddress(String str, String defaultPrefix) {
    ...
    public static AddressVersionAndBytes decode(String prefix, String address) {
        byte[] bytes = AddressCashUtil.decodeCashAddress(address, prefix);
        AddressCashValidator.checkNonEmptyPayload(bytes);

        byte extraBits = (byte) (bytes.length * 5 % 8);
        AddressCashValidator.checkAllowedPadding(extraBits);

        byte last = bytes[bytes.length - 1];
        byte mask = (byte) ((1 << extraBits) - 1);
        AddressCashValidator.checkNonZeroPadding(last, mask);

        byte[] data = new byte[bytes.length * 5 / 8];
        convertBits(data, bytes, 5, 8, false);

        byte versionByte = data[0];
        AddressCashValidator.checkFirstBitIsZero(versionByte);

        int hashSize = calculateHashSizeFromVersionByte(versionByte);
        AddressCashValidator.checkDataLength(data, hashSize);

        byte[] result = new byte[data.length - 1];
        System.arraycopy(data, 1, result, 0, data.length - 1);
        return new AddressVersionAndBytes(versionByte, result);
    }
    ...
    public static byte[] packAddressData(byte[] payload, byte type) {
